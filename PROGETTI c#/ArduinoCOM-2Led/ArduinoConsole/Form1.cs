    ...
    public partial class Form1 : Form
    {
        ...
        private void Update_Interface(int updateTime)                                           // Start updating the user interface when click the connect button
        {
            if (serialport.IsOpen)
            {
                new Thread(() =>
                {
                    ...
                    {
                        TModBusSerial.SendMessageRead(serialport, (byte)device_id.Value, ReadInputRegisters, dimRead, 3);
                    }
                }).Start();
            }
        }       
            ...
            try
            {
                if (InvokeRequired)
                {
                    this.Invoke(new MethodInvoker(delegate
                    {
                        ...
                        {
                            if (response[1] == 4)                   // update response
                            {
                                if (response[4] > 0)                // if the brightness is > than 0
                                {
                                    ...
                                    dimLed.Visible = true;
                                    dimLed.Text = dim.Value.ToString();
                                }
                            ...
                            foreach (var item in response)                                  // Console log the response
                            {
                                Console.Write($"{item}  ");
                            }
                            Console.WriteLine( );
                        ...
                        {
                            Console.WriteLine("Missing or corrupt message");
                        }
        ...
        private void On_Id_ValueChanged(object sender, EventArgs e)                             // If change the device id all the addresses use for updating the interface change for that id
        {
            ...
            foreach (var led in leds)
            {
                ...
                if(!admin)
                {
                    if (idLed.Value == led.Id && led.AdminRequire == false)
                    {
                        ledOn = led.LedOnWriteCoil;
                        ledOff = led.LedOffWriteCoil;
        ...
        private void On_dim_ValueChanged(object sender, EventArgs e)                            // If change the dim numeric set the brightness to the select value
        {
            ...
            if (serialport.IsOpen)
            {
                TModBusSerial.SendMessageWrite(serialport, (byte)device_id.Value, WriteSingleRegiste, dimWrite, setDimmerValue);
            }
        }                         
        ...
        private void OffButton_Click(object sender, EventArgs e)                                // Turn Off the led
        {
            if (serialport.IsOpen)
            {
               TModBusSerial.SendMessageWrite(serialport, (byte)device_id.Value, WriteSingleCoil, ledOff, commandOn);
            }
        }                             
        ...
        private void OnButton_Click(object sender, EventArgs e)                                 // Turn On the led
        {
            if (serialport.IsOpen)
            {
                TModBusSerial.SendMessageWrite(serialport, (byte)device_id.Value, WriteSingleCoil, ledOn, commandOn);
            }
        }                              
        ...
        private void SetMax_Click(object sender, EventArgs e)                                   // Change minimum value for the led brightness
        {
            if (serialport.IsOpen)
            {
                TModBusSerial.SendMessageWrite(serialport, (byte)device_id.Value, WriteSingleRegiste, maxDimWrite, (int)maxDimValue.Value * 255 / 100);
            }
        }                                
        ...
        private void SetMin_Click(object sender, EventArgs e)                                   // Change maximmum value for the led brightness
        {
            if (serialport.IsOpen)
            {
                TModBusSerial.SendMessageWrite(serialport, (byte)device_id.Value, WriteSingleRegiste, minDimWrite, (int)minDimValue.Value * 255 / 100);
            }
        }                                
        ...
        private void DisableControls_CheckedChanged(object sender, EventArgs e)                 // Log out from admin and hide set min and max button
        {
            ...
            {
                AdminPageControl = tabControl1.TabPages[2];
                tabControl1.TabPages.Remove(AdminPageControl);
                admin = false;
        ...
        private void SetDim_Click(object sender, EventArgs e)
        {
            if (serialport.IsOpen)
            {
                TModBusSerial.SendMessageWrite(serialport, (byte)device_id.Value, WriteSingleRegiste, dimWrite, dimWrite);
            }
        }
